#include "aes.h"
#include <stdint.h>
#include <string.h>
#include <stdlib.h>

#define Nb 4 // block size in 32-bit words
#define MAX_NR 14

// Forward S-box
static const uint8_t sbox[256] = {
    
};

static const uint8_t inv_sbox[256] = {
    
};

// Round constants
static const uint32_t Rcon[10] = {
    0x01000000,0x02000000,0x04000000,0x08000000,
    0x10000000,0x20000000,0x40000000,0x80000000,
    0x1B000000,0x36000000
};

// Helper macros
#define GETU32(pt) (((uint32_t)(pt)[0]<<24)^((uint32_t)(pt)[1]<<16)^((uint32_t)(pt)[2]<<8)^((uint32_t)(pt)[3]))
#define PUTU32(ct, st) { (ct)[0]=(uint8_t)((st)>>24); (ct)[1]=(uint8_t)((st)>>16); (ct)[2]=(uint8_t)((st)>>8); (ct)[3]=(uint8_t)(st); }

// Helper functions

void pkcs7_pad(uint8_t *buf, size_t len, size_t block) {
    uint8_t pad = block - (len % block);
    for (size_t i = 0; i < pad; i++)
        buf[len + i] = pad;
}

int pkcs7_unpad(uint8_t *buf, size_t *len) {
    uint8_t pad = buf[*len - 1];
    if (pad == 0 || pad > 16) return 0;
    for (uint8_t i = 0; i < pad; i++)
        if (buf[*len - 1 - i] != pad) return 0;
    *len -= pad;
    return 1;
}

static void gf_mul_alpha(uint8_t tweak[16]) {
    uint8_t carry = tweak[15] >> 7;
    for (int i = 15; i > 0; i--) {
        tweak[i] = (tweak[i] << 1) | (tweak[i-1] >> 7);
    }
    tweak[0] = (tweak[0] << 1) ^ (carry ? 0x87 : 0x00);
}


static void sub_bytes(uint8_t *state) {
    for(int i=0;i<16;i++) state[i]=sbox[state[i]];
}

static void inv_sub_bytes(uint8_t *state) {
    for(int i=0;i<16;i++) state[i]=inv_sbox[state[i]];
}

static void shift_rows(uint8_t *state) {
    uint8_t tmp;

    // Row 1: left rotate by 1
    tmp = state[1];
    state[1]  = state[5];
    state[5]  = state[9];
    state[9]  = state[13];
    state[13] = tmp;

    // Row 2: left rotate by 2
    tmp = state[2];
    state[2]  = state[10];
    state[10] = tmp;
    tmp = state[6];
    state[6]  = state[14];
    state[14] = tmp;

    // Row 3: left rotate by 3 (FIXED)
    tmp = state[3];
    state[3]  = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7]  = tmp;
}

static void inv_shift_rows(uint8_t *state) {
    uint8_t tmp;

    // Row 1: right rotate by 1
    tmp = state[13];
    state[13] = state[9];
    state[9]  = state[5];
    state[5]  = state[1];
    state[1]  = tmp;

    // Row 2: right rotate by 2 (FIX)
    tmp = state[2];
    state[2] = state[10];
    state[10] = tmp;
    tmp = state[6];
    state[6] = state[14];
    state[14] = tmp;

    // Row 3: right rotate by 3
    tmp = state[3];
    state[3] = state[7];
    state[7] = state[11];
    state[11] = state[15];
    state[15] = tmp;
}

static uint8_t mul(uint8_t x,uint8_t y){
    uint8_t r=0;
    while(y){
        if(y&1) r^=x;
        uint8_t hi_bit=x&0x80;
        x<<=1;
        if(hi_bit) x^=0x1b;
        y>>=1;
    }
    return r;
}


static uint8_t xtime(uint8_t x) { return (x<<1)^((x>>7)*0x1b); }

static void mix_columns(uint8_t *state) {
    for(int i=0;i<4;i++){
        uint8_t *c=state+4*i;
        uint8_t a[4]; memcpy(a,c,4);
        c[0] = xtime(a[0])^xtime(a[1])^a[1]^a[2]^a[3];
        c[1] = a[0]^xtime(a[1])^xtime(a[2])^a[2]^a[3];
        c[2] = a[0]^a[1]^xtime(a[2])^xtime(a[3])^a[3];
        c[3] = xtime(a[0])^a[0]^a[1]^a[2]^xtime(a[3]);
    }
}

static void inv_mix_columns(uint8_t *state){
    for(int i=0;i<4;i++){
        uint8_t *c=state+4*i;
        uint8_t a0=c[0],a1=c[1],a2=c[2],a3=c[3];
        c[0]=mul(a0,0x0e)^mul(a1,0x0b)^mul(a2,0x0d)^mul(a3,0x09);
        c[1]=mul(a0,0x09)^mul(a1,0x0e)^mul(a2,0x0b)^mul(a3,0x0d);
        c[2]=mul(a0,0x0d)^mul(a1,0x09)^mul(a2,0x0e)^mul(a3,0x0b);
        c[3]=mul(a0,0x0b)^mul(a1,0x0d)^mul(a2,0x09)^mul(a3,0x0e);
    }
}

static void add_round_key(uint8_t *state,const uint32_t *rk){
    for(int i=0;i<4;i++){
        uint32_t t = GETU32(state+4*i)^rk[i];
        PUTU32(state+4*i,t);
    }
}

// ================= AES Key Expansion =================
void aes_key_expansion(const uint8_t *key,int nk,AES_CTX *ctx){
    int nr = nk+6;
    ctx->nr = nr;
    uint32_t *w = ctx->round_keys;
    int i=0;
    while(i<nk) {
        w[i] = GETU32(key+4*i); i++;
    }
    while(i<4*(nr+1)){
        uint32_t temp=w[i-1];
        if(i%nk==0){
            temp = (sbox[(temp>>16)&0xff]<<24) | (sbox[(temp>>8)&0xff]<<16) | (sbox[temp&0xff]<<8) | (sbox[(temp>>24)&0xff]);
            temp ^= Rcon[i/nk-1];
        } else if(nk>6 && i%nk==4){
            temp = (sbox[(temp>>24)&0xff]<<24)|(sbox[(temp>>16)&0xff]<<16)|(sbox[(temp>>8)&0xff]<<8)|sbox[temp&0xff];
        }
        w[i]=w[i-nk]^temp;
        i++;
    }
}

// ================= AES Encrypt/Decrypt =================
void aes_encrypt(const AES_CTX *ctx,const uint8_t in[16],uint8_t out[16]){
    uint8_t state[16]; memcpy(state,in,16);
    add_round_key(state,ctx->round_keys);
    for(int round=1;round<ctx->nr;round++){
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state,ctx->round_keys+4*round);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state,ctx->round_keys+4*ctx->nr);
    memcpy(out,state,16);
}

void aes_decrypt(const AES_CTX *ctx, const uint8_t in[16], uint8_t out[16]) {
    uint8_t state[16];
    memcpy(state, in, 16);

    // Initial round key
    add_round_key(state, ctx->round_keys + 4 * ctx->nr);

    // Main rounds
    for (int round = ctx->nr - 1; round > 0; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, ctx->round_keys + 4 * round);
        inv_mix_columns(state);
    }

    // Final round (no InvMixColumns)
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, ctx->round_keys);

    memcpy(out, state, 16);
}

// CBC Encrypt / Decrypt

void aes_cbc_encrypt(
    const AES_CTX *ctx,
    uint8_t *data, size_t len,
    const uint8_t iv[16]
) {
    uint8_t prev[16];
    memcpy(prev, iv, 16);

    for (size_t i = 0; i < len; i += 16) {
        for (int j = 0; j < 16; j++)
            data[i + j] ^= prev[j];

        aes_encrypt(ctx, data + i, data + i);
        memcpy(prev, data + i, 16);
    }
}

void aes_cbc_decrypt(
    const AES_CTX *ctx,
    uint8_t *data, size_t len,
    const uint8_t iv[16]
) {
    uint8_t prev[16], tmp[16];
    memcpy(prev, iv, 16);

    for (size_t i = 0; i < len; i += 16) {
        memcpy(tmp, data + i, 16);
        aes_decrypt(ctx, data + i, data + i);

        for (int j = 0; j < 16; j++)
            data[i + j] ^= prev[j];

        memcpy(prev, tmp, 16);
    }
}

// AES CTR

static void ctr_inc(uint8_t ctr[16]) {
    for (int i = 15; i >= 0; i--) {
        if (++ctr[i]) break;
    }
}

void aes_ctr_crypt(
    const AES_CTX *ctx,
    uint8_t *data, size_t len,
    uint8_t nonce[16]
) {
    uint8_t stream[16];

    for (size_t i = 0; i < len; i += 16) {
        aes_encrypt(ctx, nonce, stream);

        size_t block = (len - i < 16) ? len - i : 16;
        for (size_t j = 0; j < block; j++)
            data[i + j] ^= stream[j];

        ctr_inc(nonce);
    }
}

// AES OFB

void aes_ofb_crypt(
    const AES_CTX *ctx,
    uint8_t *data,
    size_t len,
    uint8_t iv[16]
) {
    uint8_t feedback[16];
    memcpy(feedback, iv, 16);

    for (size_t i = 0; i < len; i += 16) {
        uint8_t stream[16];
        aes_encrypt(ctx, feedback, stream);
        memcpy(feedback, stream, 16);  // OFB feedback is encrypted output

        size_t block = (len - i < 16) ? len - i : 16;
        for (size_t j = 0; j < block; j++)
            data[i + j] ^= stream[j];
    }
}

// AES XTS

void aes_xts_encrypt(
    const AES_CTX *ctx_data,
    const AES_CTX *ctx_tweak,
    uint8_t *data,
    size_t len,       // must be >= 16
    const uint8_t tweak_iv[16]
) {
    uint8_t tweak[16];
    aes_encrypt(ctx_tweak, tweak_iv, tweak); // initial tweak

    size_t full_blocks = len / 16;
    size_t last_block = len % 16;

    for (size_t i = 0; i < full_blocks; i++) {
        uint8_t tmp[16];
        for (int j = 0; j < 16; j++) tmp[j] = data[i*16 + j] ^ tweak[j];

        aes_encrypt(ctx_data, tmp, tmp);

        for (int j = 0; j < 16; j++) data[i*16 + j] = tmp[j] ^ tweak[j];

        gf_mul_alpha(tweak);
    }

    if (last_block) {
        // Ciphertext stealing
        uint8_t tmp[16], last[16];
        memcpy(tmp, &data[(full_blocks-1)*16], 16);
        memcpy(last, &data[full_blocks*16], last_block);

        for (size_t j = 0; j < last_block; j++)
            last[j] = tmp[j];

        aes_encrypt(ctx_data, last, last);

        for (size_t j = 0; j < last_block; j++)
            data[(full_blocks-1)*16 + j] = last[j] ^ tweak[j];
    }
}

void aes_xts_decrypt(
    const AES_CTX *ctx_data,
    const AES_CTX *ctx_tweak,
    uint8_t *data,
    size_t len,
    const uint8_t tweak_iv[16]
) {
    if (len == 0) return;

    uint8_t tweak[16];
    aes_encrypt(ctx_tweak, tweak_iv, tweak); // initial tweak

    size_t nblocks = (len + 15) / 16; // ceil(len / 16)
    size_t last_block_len = len % 16;

    if (last_block_len == 0) last_block_len = 16;

    // Handle all full blocks except last if ciphertext stealing needed
    for (size_t i = 0; i < nblocks - 1; i++) {
        uint8_t tmp[16];
        for (int j = 0; j < 16; j++) tmp[j] = data[i*16 + j] ^ tweak[j];

        aes_decrypt(ctx_data, tmp, tmp);

        for (int j = 0; j < 16; j++) data[i*16 + j] = tmp[j] ^ tweak[j];

        gf_mul_alpha(tweak);
    }

    // Last block (may be partial)
    if (last_block_len == 16) {
        // Normal full block
        uint8_t tmp[16];
        for (int j = 0; j < 16; j++) tmp[j] = data[(nblocks-1)*16 + j] ^ tweak[j];
        aes_decrypt(ctx_data, tmp, tmp);
        for (int j = 0; j < 16; j++) data[(nblocks-1)*16 + j] = tmp[j] ^ tweak[j];
    } else {
        // Ciphertext stealing
        uint8_t prev[16], last[16];
        memcpy(prev, &data[(nblocks-2)*16], 16);
        memcpy(last, &data[(nblocks-1)*16], last_block_len);

        // Swap tail bytes from prev to last
        for (size_t j = 0; j < last_block_len; j++)
            prev[16 - last_block_len + j] = last[j];

        uint8_t tmp[16];
        for (int j = 0; j < 16; j++) tmp[j] = prev[j] ^ tweak[j];
        aes_decrypt(ctx_data, tmp, tmp);
        for (int j = 0; j < 16; j++) prev[j] = tmp[j] ^ tweak[j];

        // Write back
        memcpy(&data[(nblocks-2)*16], prev, 16);
        memcpy(&data[(nblocks-1)*16], &prev[16 - last_block_len], last_block_len);
    }
}